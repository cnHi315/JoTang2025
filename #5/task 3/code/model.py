...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((3, 3)) # Avgpool layer
        self.fc = nn.Linear(720, 10) # Linear layer
    
    def forward(self, x, intermediate_outputs=False):
        x = self.conv1(x)
        conv1_out = x  # 保存第一个卷积层的输出
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
        conv2_out = x  # 保存第二个卷积层的输出
        x = self.bn2(x)
        x = self.relu(x)
        x = self.maxpool(x)
        x = self.conv3(x)
        conv3_out = x  # 保存第三个卷积层的输出
        x = self.bn3(x)
        x = self.relu(x)
        x = self.maxpool(x)
        x = self.conv4(x)
        conv4_out = x  # 保存第四个卷积层的输出
        x = self.bn4(x)
        x = self.relu(x)
        x = self.maxpool(x)
        x = self.conv5(x)
        conv5_out = x  # 保存第五个卷积层的输出
        x = self.avgpool(x)
